...
def _parse_meds_from_text(text: str) -> List[MedItem]:
    """
    Extraction naïve via regex : lignes du type
    - "Paracétamol 1/jour"
    - "Ibuprofène 2/jour"
    ...
    for raw in [l.strip() for l in text.splitlines() if len(l.strip()) > 2]:
        ...
        for pat, fmt in FREQ_PATTERNS:
            ...
            if m:
                ...
                break
        if not freq:
            # autre pattern type "1 / jour ?" (ton mock)
...
def extract_text_from_image(image_bytes: bytes) -> str:
    if not OCR_AVAILABLE:
        # fallback : si on n’a pas d’OCR, on retournera une chaîne vide
        return ""
...
def extract_meds(image_bytes: Optional[bytes], typed_text: Optional[str] = None) -> List[Dict]:
    ...
    return [ {"nom": m.nom, "frequence": m.frequence} for m in meds ]
